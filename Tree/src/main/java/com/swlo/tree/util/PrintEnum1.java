...
public enum PrintEnum1 {
    ...
    PrintEnum1(TreeFunction function) {
        this.function = function;
    }

    private static <T extends Comparable<T>> List<T> print(AvlTreeNode<T> node, Order order) {
        ...
        switch (order) {
            ...
            case LEVEL:
                ...
                queue.add(node);
                while (!queue.isEmpty()) {
                    AvlTreeNode<T> currentNode = queue.poll();
    ...
    public interface TreeFunction<T extends Comparable<T>> {
        List<T> apply(AvlTreeNode<T> tree);
    }
}