...
public enum PrintEnum2 {
    ...
    PrintEnum2(TreeFunction function) {
        this.function = function;
    }

    private static <T extends Comparable<T>> List<T> print(RedBlackTreeNode<T> node, Order order) {
        ...
        switch (order) {
            ...
            case LEVEL:
                ...
                queue.add(node);
                while (!queue.isEmpty()) {
                    RedBlackTreeNode<T> currentNode = queue.poll();
    ...
    public interface TreeFunction<T extends Comparable<T>> {
        List<T> apply(RedBlackTreeNode<T> tree);
    }
}